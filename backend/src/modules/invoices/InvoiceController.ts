...
class InvoiceController {
    async handleFindInvoiceByClient(
        request: Request,
        response: Response
    ): Promise<Response> {
        try {
            ...
            const installations = await installationRepository.findByClientId(
                client,
                true
            );
            ...
            return response.status(200).json({
                ...
                data: installations.flatMap(
                    installation => installation.invoices
                ),
            });
        } catch (error) {
    ...
    async handleFindInvoicePathByClient(
        request: Request,
        response: Response
    ): Promise<Response> {
        try {
            ...
            for (const installation of installations) {
                ...
                if (list.length > 0) {
                    paths.push({
                        installation: installation.installationCode,
                        files: list,
                    });
                }
            }
